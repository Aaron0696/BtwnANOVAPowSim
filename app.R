...
ui <- fluidPage(
    ...
    fluidRow(
        ...
        column(wellPanel(h3("Group Means"), 
                         "Input the unstandardized group means here, larger differences between means lead to higher power.",
                         hr(),
                         uiOutput("grpmeans")), width = 2),
        column(wellPanel(h3("Group SDs"), 
                         "Input the group standard deviations (SDs) here, smaller SDs between means lead to higher power. One assumption usually made is that variances are equal across groups but feel free to change it.",
                         hr(),
                         uiOutput("grpsds")), width = 2),
        column(wellPanel(h3("Group Size"), 
                         "Input the group sizes here, equal group sizes are ideal to maximize power. One assumption usually made is that group sizes are equal but feel free to change it.",
                         hr(),
